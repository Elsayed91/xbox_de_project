...
def test_clean_tweet():
    ...
    assert (
        clean_tweet("I'm excited to go to the park!") == "im excited to go to the park"
    )
    ...
    assert (
        clean_tweet("This tweet contains a @mention and a #hashtag.")
        == "this tweet contains a and a"
    )

    # Test removal of URLs
    assert (
        clean_tweet("Check out this link: https://www.example.com.")
        == "check out this link"
    )
    ...
    assert (
        clean_tweet("This tweet contains stopwords.", stopwords=["contains"])
