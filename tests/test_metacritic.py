...
def test_game_urls():
    ...
    urls = list(game_urls("https://www.metacritic.com/browse/games/release-date/available/switch/name?&view=detailed", 2))
    assert len(urls) == 200  # There should be 200 game URLs on the first two pages for the Nintendo Switch
    assert all(url.startswith("https://www.metacritic.com/game/") for url in urls)  # All URLs should be for games on Metacritic
...
def test_get_games_per_page():
    ...
    with patch('scrapers.metacritic.scrape_utils.soup_it') as mock_soup_it:
        ...
        expected_output = ["https://example.com/game1", "https://example.com/game2", "https://example.com/game3"]
